...
CustomerEditorDialog::CustomerEditorDialog(QWidget* parent)
...
void CustomerEditorDialog::buildUi()
{
    ...
    connect(btnSave,   &QPushButton::clicked, this, &CustomerEditorDialog::accept);
    connect(btnCancel, &QPushButton::clicked, this, &CustomerEditorDialog::reject);
...
void CustomerEditorDialog::setForAdd(unsigned short int nextId)
{
    ...
    clearErrors();
    setWindowTitle(QString("New Customer (ID %1)").arg(nextId));
}

void CustomerEditorDialog::setForEdit(const Customer& existing)
{
    m_id = existing.getId();
...
void CustomerEditorDialog::clearErrors()
{
...
void CustomerEditorDialog::accept()
{
    clearErrors();
    ...
    try {
        ...
        m_result = Customer(CustomerData{
            m_id,
            nameBytes.constData(),
