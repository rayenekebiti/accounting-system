...
SavingsAccount::SavingsAccount(uint32_t id, const std::string& name,
...
void SavingsAccount::serialize(char* buf) const
{
    ...
    std::memcpy(buf + 113, &intrestRate,          sizeof(intrestRate));
    std::memcpy(buf + 117, &withdrawalsThisMonth, sizeof(withdrawalsThisMonth));
}

void SavingsAccount::deserialize(const char* buf)
{
    BankAccount::deserialize(buf);
    std::memcpy(&intrestRate,          buf + 113, sizeof(intrestRate));
    std::memcpy(&withdrawalsThisMonth, buf + 117, sizeof(withdrawalsThisMonth));
}
